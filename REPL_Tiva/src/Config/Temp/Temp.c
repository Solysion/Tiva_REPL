...
uint32_t ADC0Vector; 				// Sample buffer (size = number of samples)

static float TempCelsius; 			// Temperature variable
static uint32_t temp_intpart;		// Integer part of temperature
static uint32_t temp_decpart;		// Decimal part of temperature
...
void ConfigLib_Temp_Measure(void)
{
	...
	ADCSequenceDataGet(ADC0_BASE, ADC_SEQUENCE, &ADC0Vector);

	// Convert binary to float value
	TempCelsius = (1475 - ((2475 * ADC0Vector)) / 4096)/10;

	// Split integer and decimal parts from float value
	temp_intpart = (uint32_t) TempCelsius;
	temp_decpart = (uint32_t)(TempCelsius * 10) - temp_intpart;
