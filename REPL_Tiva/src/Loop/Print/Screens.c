...
void CoreClock_Screen(void)
{
	...
	UARTprintf("%d",(char *)ConfigLib_CoreClock_Get());
	UARTprintf(" Hz.");
	UARTprintf("\n");
}

// 2.8. Temperature screen;

void CoreTemp_Screen(void)
{
	UARTprintf("Temperature now is ");
	UARTprintf("%d",(char *)TempCelsius_IntegerPart_Read());	// Print integer part of temperature;
	UARTprintf(".");
	UARTprintf("%d",(char *)TempCelsius_DecimalPart_Read());	// Print decimal part of temperature;
